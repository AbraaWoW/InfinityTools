...
local WAImportsModule        = RRT_NS.UI.Options and RRT_NS.UI.Options.WAImports or nil
local BuildWAImportsOptions  = WAImportsModule and WAImportsModule.BuildOptions or function() return {} end
local BuildWAImportsCallback = WAImportsModule and WAImportsModule.BuildCallback or function() return function() end end
local BuildWAImportsUI       = WAImportsModule and WAImportsModule.BuildUI or nil
...
function RRTUI:Init()
    ...
    -- Main tabs: General contains a sidebar for General / Nicknames / Versions
    local tabList = TABS_LIST()
    local tabContainer = DF:CreateTabContainer(RRTUI, "Infinity", "RRTUI_TabsTemplate", tabList, {
        width = window_width,
    ...
    local tools_tab         = tabContainer:GetTabFrameByName("Tools")
    local boss_tab          = tabContainer:GetTabFrameByName("Boss")
    ...
    local waimports_options_table = BuildWAImportsOptions()
    local waimports_callback = BuildWAImportsCallback()
    ...
    do
        ...
        for _, sec in ipairs(SECTIONS) do
            ...
            elseif sec.key == "versions" then
                RRTUI.version_scrollbox = BuildVersionCheckUI(panel)
            end
    ...
    if BuildWAImportsUI then
        BuildWAImportsUI(waimports_tab)
    else
        DF:BuildMenu(waimports_tab, waimports_options_table, 10, -10, window_height - 10, false, options_text_template,
            options_dropdown_template, options_switch_template, true, options_slider_template, options_button_template,
            waimports_callback)
    end
    ...
    -- Boss tab — opens the InfinityBoss settings panel
    do
        local btn = DF:CreateButton(boss_tab,
            function()
                RRTUI:Hide()
            ...
            220, 30, "Open InfinityBoss Settings",
            nil, nil, nil, nil, nil, false,
            options_button_template, options_text_template)
        btn:SetPoint("CENTER", boss_tab, "CENTER", 0, 0)
    end

    -- Tools tab — opens the InfinityTools settings panel
    do
    ...
    local versionNumber = " v"..C_AddOns.GetAddOnMetadata("InfinityTools", "Version")
    --[==[@debug@
