...
local checkable_components = {"Addon", "Note", "Reminder", "InfinityTools"}
...
local function _UpdateCheckButton()
    ...
    -- InfinityTools check is open to everyone (checks own install)
    if component_type == "InfinityTools"
...
local function BuildVersionCheckUI(parent)
    ...
    hide_version_response_button:SetPoint("TOPLEFT", parent, "TOPLEFT", 10, -12)
    ...
    component_type_label:SetPoint("TOPLEFT", parent, "TOPLEFT", 10, -42)
    ...
    local component_name_entry = DF:CreateTextEntry(parent, function(_, _, value) component_name = value end, 180, 18)
    ...
    version_check_button:SetPoint("LEFT", component_name_entry, "RIGHT", 10, 0)
    ...
    character_name_header:SetPoint("TOPLEFT", parent, "TOPLEFT", 20, -108)

    local version_number_header = DF:CreateLabel(parent, "Version Number", 11)
    version_number_header:SetPoint("LEFT", character_name_header, "RIGHT", 175, 0)

    local ignore_header = DF:CreateLabel(parent, "Ignore Check", 11)
    ignore_header:SetPoint("LEFT", version_number_header, "RIGHT", 95, 0)
        ...
        for i = 1, totalLines do
            ...
            if thisData then
                line:SetScript("OnClick", function(self)
                    ...
                    if vc.type == "InfinityTools" then
                        if thisData.version == "Not Installed" then message = "Please install InfinityTools"
                        elseif thisData.version == "Not Enabled" then message = "Please enable InfinityTools"
                        elseif thisData.version == "No Response" then return
                        else message = "Please update InfinityTools" end
    ...
    local function createLineFunc(self, index)
        ...
        version:SetPoint("LEFT", name, "RIGHT", 15, 0)
        ...
        ignorelist:SetPoint("LEFT", version, "RIGHT", 10, 0)
    ...
    local SCROLL_Y    = -132
    local SCROLL_H    = window_height - 122 - math.abs(SCROLL_Y) - 8
    local scrollLines = math.floor(SCROLL_H / 21)
    local SCROLL_W    = window_width - 285
    ...
    version_check_button:SetScript("OnClick", function(self)
        ...
        local component_type = component_type_dropdown:GetValue()
        local isNoName = component_type == "Note" or component_type == "Reminder" or component_type == "InfinityTools"
        ...
        local checkName = (component_type == "InfinityTools") and "InfinityTools" or text
    ...
    version_presets_edit_frame:SetPoint("CENTER", UIParent, "CENTER", 0, 0)
    ...
    version_presets_edit_button:SetPoint("TOPLEFT", parent, "TOPLEFT", 285, -72)
    version_presets_edit_button:SetTemplate(options_button_template)
    version_check_preset_dropdown:SetPoint("LEFT", parent, "TOPLEFT", 55, -72)
    preset_label:SetPoint("LEFT", parent, "TOPLEFT", 10, -72)
