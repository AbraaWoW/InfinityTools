...
function RRT_NS:RequestVersionNumber(type, name) -- type == "Addon" or "WA" or "Note"
    if type == "InfinityTools" or (UnitIsGroupLeader("player") or UnitIsGroupAssistant("player") or RRT.Settings["Debug"]) then
...
function RRT_NS:VersionResponse(data)
    self.RRTUI.version_scrollbox:AddData(data)
end


function RRT_NS:GetVersionNumber(type, name, unit)
    ...
    if type == "InfinityTools" then
        local ver = C_AddOns.GetAddOnMetadata("InfinityTools", "Version") or "Not Installed"
        if ver ~= "Not Installed" then
            ver = C_AddOns.IsAddOnLoaded("InfinityTools") and ver or "Not Enabled"
        end
