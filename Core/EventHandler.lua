...
function RRT_NS:EventHandler(e, wowevent, internal, ...) -- internal checks whether the event comes from addon comms. We don't want to allow blizzard events to be fired manually
    if e == "ADDON_LOADED" and wowevent then
        ...
        if name == "InfinityTools" then
            ...
            if RRT.ReminderSettings.enabled == nil then RRT.ReminderSettings.enabled = false end
            ...
            if not RRT.ReminderSettings.PersonalReminderFrame then
                RRT.ReminderSettings.PersonalReminderFrame = {enabled = false, Width = 500, Height = 600, Anchor = "TOPLEFT", relativeTo = "TOPLEFT", xOffset = 500, yOffset = 0, Font = "Expressway", FontSize = 14, BGcolor = {0, 0, 0, 0.3},}
            end
    ...
    elseif e == "PLAYER_LOGIN" and wowevent then
        self.RRTUI:Init()
        if RRT_NS.Tools and RRT_NS.Tools.Init then
            RRT_NS.Tools:Init()
        end
        self:InitLDB()
        self:InitQoL()
        ...
        -- Hook InfinityTools:ToggleGlobalEditMode so that enabling/disabling
        -- InfinityBoss edit mode also syncs all RRT reminder movers in one click.
        C_Timer.After(0, function()
            local ET = _G.InfinityTools
            if ET and ET.ToggleGlobalEditMode and not ET._rrtEditModeHooked then
                ET._rrtEditModeHooked = true
                local orig = ET.ToggleGlobalEditMode
                ET.ToggleGlobalEditMode = function(self, ...)
                    orig(self, ...)
                    local on = self.GlobalEditMode == true
                    -- reminder display movers (Icon / Bar / Text)
                    for _, name in ipairs({ "IconMover", "BarMover", "TextMover" }) do
                        if RRT_NS[name] then
                            RRT_NS:ToggleMoveFrames(RRT_NS[name], on)
                        end
                    end
                    -- note / text-display movers (Manage Reminders)
                    for _, name in ipairs({ "ReminderFrameMover", "PersonalReminderFrameMover", "ExtraReminderFrameMover" }) do
                        if RRT_NS[name] then
                            RRT_NS:ToggleMoveFrames(RRT_NS[name], on)
                        end
                    end
