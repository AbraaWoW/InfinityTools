...
local allowedcomms = {
    ["RRT_NICKNAMES_COMMS"] = true,
    ["RRT_NICKNAMES_SYNC"] = true,
}

local del = ":"
-- disabling this for now as weakauras is gone but maybe in the future I need it again
--[[
function RRTAPI:Broadcast(event, channel, ...)
    local message = event
    local argTable = {...}
    local target = ""
...
function RRT_NS:Broadcast(event, channel, ...) -- using internal broadcast function for anything inside the addon to prevent users to send stuff they shouldn't be sending
    local message = event
    local argTable = {...}
    local target = ""
    ...
    if channel == "WHISPER" then -- create "fake" whisper addon msg that actually just uses RAID/PARTY instead and will be checked on receive
        if IsInRaid() then
            AceComm:SendCommMessage("RRT_WHISPER", message, "RAID")
        elseif IsInGroup() then
            AceComm:SendCommMessage("RRT_WHISPER", message, "PARTY")
        end
    else
        AceComm:SendCommMessage("RRT_MSG", message, channel)
    end
end
...
local function ReceiveComm(text, chan, sender, whisper, internal)
    ...
    if (UnitExists(sender) and (UnitInRaid(sender) or UnitInParty(sender))) or (chan == "GUILD" and allowedcomms[event]) then -- block addon msg's from outside the raid, only exception being the guild nickname comms.
        ...
        RRT_NS:EventHandler(event, false, internal, unpack(formattedArgTable))
...
AceComm:RegisterComm("RRT_MSG", function(_, text, chan, sender) ReceiveComm(text, chan, sender, false, true) end)
AceComm:RegisterComm("RRT_WHISPER", function(_, text, chan, sender) ReceiveComm(text, chan, sender, true, true) end)