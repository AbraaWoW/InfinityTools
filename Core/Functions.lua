...
function RRT_NS:IterateGroupMembers(reversed, forceParty)
...
function RRT_NS:Restricted()
    return C_Secrets.ShouldAurasBeSecret()
end

function RRT_NS:Print(...)
    if RRT.Settings["DebugLogs"] then
        if DevTool then
            local t = {...}
            local name = t[1]
...
function RRT_NS:SortTable(t, reversed)
    table.sort(t,
        function(a, b)
...
function RRTAPI:Shorten(unit, num, specicon, AddonName, combined, roleicon) -- Returns color coded Name/Nickname
    if issecretvalue(unit) or not unit then return unit, "", "" end
    local classFile = unit and select(2, UnitClass(unit))
    if specicon then
        local specid = 0
        if unit then specid = RRT_NS:GetSpecs(unit) or 0 end
    ...
    if classFile then -- basically "if unit found"
        ...
        name = num and RRT_NS:Utf8Sub(RRTAPI:GetName(name, AddonName), 1, num) or RRTAPI:GetName(name, AddonName) -- shorten name before wrapping in color
...
function RRT_NS:GetSpecs(unit)
    if unit then
        return RRT_NS.specs[unit] or false -- return false if no information available for that unit so it goes to the next fallback
    else
        return RRT_NS.specs -- if no unit is given then entire table is requested
    end
end
...
function RRT_NS:IsMelee(unit)
    local role = UnitGroupRolesAssigned(unit)
    if role == "TANK" then return true end
    if unit == "player" then
        local spec = GetSpecializationInfo(GetSpecialization())
        return self.meleetable[spec] or false
    else
        local spec = self:GetSpecs(unit) or 0
        return (spec ~= 0 and self.meleetable[spec]) or false
    end
end


function RRT_NS:GetNote() -- simply for note comparison now
...
function RRT_NS:DifficultyCheck(num) -- check if current difficulty is a Normal/Heroic/Mythic raid and also allow checking if we are currently in an encounter
    local difficultyID = select(3, GetInstanceInfo()) or 0
    return ((difficultyID >= num and difficultyID <= 16 and difficultyID)) or (RRT.Settings["Debug"] and 16)
end

function RRT_NS:GetHash(text)
    local counter = 1
...
-- GetChar / GetName: NickNames removed, return name as-is
function RRTAPI:GetChar(name, returnUnit)
    return name
end

function RRTAPI:GetName(name, addonName)
    return name
end

function RRTAPI:GetSpecs(unit)
    return RRT_NS:GetSpecs(unit)
end

-- keeping these two in global as I might want to use them elsewhere still
function RRTAPI:TTSCountdown(num)
    for i= num, 1, -1 do
        if i == num then
            RRTAPI:TTS(i)
        else
            C_Timer.After(num-i, function() RRTAPI:TTS(i) end)
        end
    end
end

local path = "Interface\\AddOns\\InfinityTools\\Media\\Sounds\\"
function RRTAPI:TTS(sound, voice, overlap) -- RRTAPI:TTS("Bait Frontal")
    if RRT.Settings["TTS"] then
        ...
        if handle then
            ...
            local num = voice or RRT.Settings["TTSVoice"]
...
function RRT_NS:GetSubGroup(unit)
    for i=1, 40 do
...
function RRT_NS:SpecToName(specid)
...
function RRT_NS:Utf8Sub(str, startChar, endChar)
...
function RRT_NS:UnitAura(unit, spell) -- simplify aura checking for myself
...
RRT_NS.Callbacks = RRT_NS.Callbacks or LibStub("CallbackHandler-1.0"):New(RRT_NS)

function RRT_NS:FireCallback(event, ...)
    self.Callbacks:Fire(event, ...)
end

function RRTAPI:RegisterCallback(event, callback, owner)
    return RRT_NS:RegisterCallback(event, callback, owner)
end

function RRTAPI:UnregisterCallback(event, callback, owner)
    return RRT_NS:UnregisterCallback(event, callback, owner)
end

function RRTAPI:UnregisterAllCallbacks(owner)
    return RRT_NS:UnregisterAllCallbacks(owner)
end

local Serialize = LibStub("AceSerializer-3.0")
local Compress = LibStub("LibDeflate")

function RRT_NS:CreateExportString(SettingsTable) -- {"ReminderSettings", "PASettings", ...}
    local str = ""
    local ExportTable = {}
...
function RRT_NS:ImportFromTable(ImportTable)
    local changed = false
...
function RRT_NS:ImportSettingsFromString(string)
...
function RRT_NS:StopFrameMove(F, SettingsTable)
...
function RRT_NS:ToggleMoveFrames(F, Unlock)
    if not F then return end
    if Unlock then
        F:SetMovable(true)
        F:EnableMouse(true)
        ...
        if F.Label then F.Label:Show() end
    else
