-- =============================================================
-- InfinityTools_Changelog.lua
-- Core changelog system:
-- 1. Save the current version into WTF
-- 2. Use WTF state to decide whether to show a popup when opening the settings panel (once per version)
...
if not MODULE_DB then return end

MODULE_DB.Changelog = MODULE_DB.Changelog or {}
local CL_DB = MODULE_DB.Changelog
...
local function GetCurrentVersion()
    return tostring(InfinityTools.VERSION or (_G.InfinityTools_MetaData and _G.InfinityTools_MetaData.version) or "v0.0.0.0000")
end

local function GetMetaChangelog()
    local meta = _G.InfinityTools_MetaData
...
local function ResolveLineStyle(line, baseSize)
    ...
    if h1 then
        -- Backward compatibility: strip trailing date/time from H1 titles when displaying old content
    ...
    if line:match("^%s*PS:") or line:match("^%s*Note:") then
...
local function EnsureChangelogFrame()
    ...
    changelogFrame = CreateFrame("Frame", "InfinityToolsChangelogFrame", UIParent, "BackdropTemplate")
    ...
    tinsert(UISpecialFrames, "InfinityToolsChangelogFrame")

    -- Apply ElvUI / NDui skins.
    -- EnsureChangelogFrame is first called after PLAYER_ENTERING_WORLD, so skin modules should be initialized.
    local f = changelogFrame
    local ElvUISkin = InfinityTools.ElvUISkin
    ...
    if ElvUISkin and ElvUISkin:IsElvUILoaded() then
        ...
        if S then
            pcall(function()
                -- Main background: use ElvUI's Transparent template instead of the custom backdrop
                ...
                -- ScrollBar created by UIPanelScrollFrameTemplate
    ...
    elseif NDuiSkin and NDuiSkin:IsNDuiLoaded() then
        ...
        if B then
            pcall(function()
                -- Main background: apply NDui backdrop directly on f using SkinAlpha
                B.CreateBD(f)
                -- Shadow
                B.CreateSD(f, nil, true)
                -- Background texture
                B.CreateTex(f)
                -- Close button, must use ReskinClose or the X icon may disappear
...
function InfinityTools:ShowChangelog(options)
    options = options or {}
    MarkSeenVersion()
