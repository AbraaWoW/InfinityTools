...
local function GetSpecIndexByID(specID)
...
local function SaveCurrentLoadout(data)
    if not data or not db() then return false end
    ...
    if IsTLXAvailable() then
        ...
        if tlxName then
            ...
            return true, string.format("Saved TLX loadout for |cFFFF9900%s|r", data.name)
        end
        return false, "|cFFFF4444No TLX loadout active. Select a loadout first.|r"
    end
    ...
    return true, string.format("Saved |cFFFF9900%s|r for %s", configName, data.name)
end

local function FinalizeLoadoutSwap(saved)
    local targetSpecID = saved and saved.specID or 0
    local activeSpecID = GetSpecID()

    if targetSpecID ~= 0 and activeSpecID ~= 0 and targetSpecID ~= activeSpecID then
        ChatMsg(string.format("|cFFFF4444Wrong active spec.|r Current: %s, Saved: %s",
            GetSpecName(), saved.specName or tostring(targetSpecID)))
        return false
    end

    local configs = C_ClassTalents.GetConfigIDsBySpecID(activeSpecID) or {}
    ...
    ChatMsg("|cFFFF4444Loadout not found for the active spec.|r")
    return false
end
...
local function SwapToSaved(saved)
    ...
    local targetSpecID = saved and saved.specID or 0
    local currentSpecID = GetSpecID()
    if targetSpecID ~= 0 and currentSpecID ~= 0 and targetSpecID ~= currentSpecID then
        local targetSpecIndex = GetSpecIndexByID(targetSpecID)
        if not targetSpecIndex or not SetSpecialization then
            ChatMsg("|cFFFF4444Cannot switch to the saved specialization automatically.|r")
            return false
        end

        SetSpecialization(targetSpecIndex)
        ChatMsg(string.format("Switching spec to |cFFFF9900%s|r...", saved.specName or tostring(targetSpecID)))
        C_Timer.After(1.0, function()
            if not InCombatLockdown() then
                FinalizeLoadoutSwap(saved)
            end
        end)
        return true
    end

    return FinalizeLoadoutSwap(saved)
end
...
StaticPopupDialogs["RRT_TALENT_SAVE"] = {
    ...
    OnAccept = function(self)
        ...
        local ok, message = SaveCurrentLoadout(data)
        if message then ChatMsg(message) end
        if ok then TriggerUIRebuild() end
    end,
...
StaticPopupDialogs["RRT_TALENT_MISMATCH"] = {
    ...
    OnCancel = function(self)
        ...
        local ok, message = SaveCurrentLoadout(data)
        if ok and data then
            message = string.format("Updated saved build for |cFFFF9900%s|r", data.name)
        end
        if message then ChatMsg(message) end
        if ok then TriggerUIRebuild() end
    end,
...
-- Core check
-- ─────────────────────────────────────────────────────────────────────────────
local function GetReminderContext()
    ...
    local displayName = zoneName or "Unknown"
    local diffName = difficultyName

    if instanceType == "party" and (difficulty == 8 or difficulty == 23) then
        return {
            key = string.format("%d:party:%d:%d", specID, instanceID or 0, difficulty),
            displayName = displayName ~= "" and displayName or "Mythic+",
            diffName = diffName,
        }
    end
    ...
    if instanceType == "raid" then
        return {
            key = string.format("%d:raid:%d:%d", specID, instanceID or 0, difficulty or 0),
            displayName = displayName ~= "" and displayName or "Raid",
            diffName = diffName,
        }
    end
...
local function CheckTalents(key, displayName, diffName)
    ...
    if not saved then
        ...
        local promptText = "|cFFBB66FFInfinity Raid Tools|r\n\n"
        ...
        if isMismatch then
            local activeSpecName = GetSpecName()
            local savedSpecName = saved.specName or "Unknown"
            local promptText = "|cFFBB66FFInfinity Raid Tools|r\n\n"
                .. string.format("Talent mismatch for |cFFFF9900%s|r!\n\nActive spec:  |cFFFF4444%s|r\nSaved spec:   |cFF00FF00%s|r\n\nActive build: |cFFFF4444%s|r\nSaved build:  |cFF00FF00%s|r",
                    displayName, activeSpecName, savedSpecName, configName or "?", savedDisplayName)
...
local function OnZoneChanged()
    ...
    local context = GetReminderContext()
    if not context then
        lastCheckedZone = nil
        return
    end
    ...
    C_Timer.After(1, function()
        if not InCombatLockdown() then
            CheckTalents(key, context.displayName, context.diffName)
        end
    end)
end
