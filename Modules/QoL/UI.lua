...
local function BuildQoLUI(parent)
    ...
    local combatSubDefs = {
        { name = L["qol_combat_durability"],   key = "durability"   },
    ...
    if OptDU then
        DF:BuildMenu(combatSubPanels["durability"], OptDU.BuildOptions(), 10, -10,
            subMenuHeight, false,
            options_text_template, options_dropdown_template, options_switch_template,
    ...
    if OptTR then
        ...
        trDesc:SetText("|cFFAAAAAAWhen you enter a supported Mythic+ dungeon or raid, your current talents are compared with the saved build. A popup lets you swap, update, or ignore the reminder.|r")
        trDesc:SetJustifyH("LEFT")
        trDesc:SetJustifyV("TOP")
        ...
        if not StaticPopupDialogs["RRT_TALENT_CLEAR_CONFIRM"] then
            StaticPopupDialogs["RRT_TALENT_CLEAR_CONFIRM"] = {
                text    = "|cFFBB66FFInfinity Raid Tools|r\n\n"
                    .. "|cFFFF4444This will delete ALL saved talent builds.|r\n"
                    .. "You will be prompted again the next time you enter a supported Mythic+ dungeon or raid.\n\n"
                    .. "Are you sure?",
                button1 = "Clear All",
                button2 = "Cancel",
                OnAccept = function()
    ...
    local mplusSubDefs = {
        ...
        { name = L["qol_mplus_groupjoined"],   key = "groupjoined"   },
    }
    ...
    local OptGJ = RRT_NS.UI.Options.MP_GroupJoined
    if OptGJ then
        DF:BuildMenu(mplusSubPanels["groupjoined"], OptGJ.BuildOptions(), 10, -10,
            subMenuHeight, OptGJ.singleColumn or true,
            options_text_template, options_dropdown_template, options_switch_template,
            true, options_slider_template, options_button_template, OptGJ.BuildCallback())
    end
