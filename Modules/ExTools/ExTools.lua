...
local function GetDB()
    RRT.ExTools = RRT.ExTools or {}
    local db = RRT.ExTools
    ...
    if db.ResetDMG == nil then db.ResetDMG = false end
...
-- Frame handles
-- ═══════════════════════════════════════════════════════════════
local ExToolsFrame = CreateFrame("Frame", "RRT_ExToolsFrame")
...
local function InitAutoTools()
    ...
    ExToolsFrame:RegisterEvent("DELETE_ITEM_CONFIRM")

    -- Merchant: sell junk + repair
    ExToolsFrame:RegisterEvent("MERCHANT_SHOW")
end
...
local function InitMapInfo()
    ...
    MapInfoFrame = CreateFrame("Frame", "RRT_ExTools_MapInfoFrame", WorldMapFrame)
...
local function InitRangeCheck()
    ...
    RangeCheckFrame = CreateFrame("Frame", "RRT_ExTools_RangeCheckFrame", UIParent)
    local db = GetDB()
    ...
    RangeCheckFrame:SetScript("OnUpdate", function(self, dt)
        ...
        if rangeSpell and rangeSpell > 0 then
            ...
            -- Approximate range using CheckInteractDistance
            local melee = CheckInteractDistance("target", 3) -- interact = 5y
            local close = CheckInteractDistance("target", 2) -- ~10y
            local mid   = CheckInteractDistance("target", 1) -- ~28y
            if melee then
                displayVal = "<5y"
                r, g, b = 0.9, 0.9, 0.9
            elseif close then
...
local function InitPlayerPosition()
    ...
    if not PlayerPosFrame then
        PlayerPosFrame = CreateFrame("Frame", "RRT_ExTools_PlayerPosFrame", UIParent)
...
local function InitSpellQueue()
    ApplySpellQueue()
    ExToolsFrame:RegisterEvent("PLAYER_SPECIALIZATION_CHANGED")
end
...
local function InitYYSound()
    ...
    YYSoundFrame = CreateFrame("Frame", "RRT_ExTools_YYSoundFrame", UIParent)
    YYSoundFrame:SetSize(sz, sz)
    YYSoundFrame:SetPoint("CENTER", UIParent, "CENTER", db.YYPosX or -390, db.YYPosY or 14)
    ...
    YYCooldown = CreateFrame("Cooldown", "RRT_ExTools_YYCooldown", YYSoundFrame, "CooldownFrameTemplate")
...
local function InitBulkBuy()
    ...
    BulkBuyPanel = CreateFrame("Frame", "RRT_ExTools_BulkBuyPanel", UIParent, "BackdropTemplate")
    ...
    local qtyBox = CreateFrame("EditBox", "RRT_ExTools_BulkBuyQtyBox", BulkBuyPanel, "InputBoxTemplate")
...
-- ─── Event Handler ───────────────────────────────────────────
-- ═══════════════════════════════════════════════════════════════
ExToolsFrame:SetScript("OnEvent", function(self, event, ...)
    local db = GetDB()
    ...
    elseif event == "ZONE_CHANGED_NEW_AREA" then
        if db.ResetDMG then
            local inInst = select(1, IsInInstance()) == true or IsInInstance()
            if inInst and not _wasInInstance then
                StaticPopupDialogs["RRT_RESET_DAMAGE_METER"] = {
                    text = "Reset damage meter for new instance?",
                    button1 = "Reset",
                    button2 = "Skip",
                    OnAccept = function()
                ...
                StaticPopup_Show("RRT_RESET_DAMAGE_METER")
            end
            _wasInInstance = inInst
        end
    ...
    elseif event == "UNIT_SPELLCAST_SUCCEEDED" then
        ...
        local link = C_Spell.GetSpellLink(spellID) or GetSpellInfo(spellID) or tostring(spellID)
    ...
    elseif event == "UNIT_AURA" then
        ...
        for buffID in pairs(BLOODLUST_SET) do
            ...
            if aura then
                ...
                if expiry ~= (ExToolsFrame._lastYYExpiry or 0) then
                    ExToolsFrame._lastYYExpiry = expiry
                    TriggerYYSound()
                end
                break
            end
...
local function RefreshAll()
    ...
    -- TeleMsg events
    ExToolsFrame:UnregisterEvent("UNIT_SPELLCAST_SUCCEEDED")
    ExToolsFrame:UnregisterEvent("UNIT_SPELLCAST_START")
    if db.TeleMsg then
        if db.TelemsgOnSuccess then
            ExToolsFrame:RegisterEvent("UNIT_SPELLCAST_SUCCEEDED")
        else
            ExToolsFrame:RegisterEvent("UNIT_SPELLCAST_START")
        end
    end

    -- UNIT_AURA for YY
    ExToolsFrame:UnregisterEvent("UNIT_AURA")
    if db.YYSound then
        ExToolsFrame:RegisterEvent("UNIT_AURA")
    end
...
local function Init()
    ...
    ExToolsFrame:RegisterEvent("DELETE_ITEM_CONFIRM")
    ExToolsFrame:RegisterEvent("MERCHANT_SHOW")
    ExToolsFrame:RegisterEvent("ZONE_CHANGED_NEW_AREA")
    ExToolsFrame:RegisterEvent("PLAYER_SPECIALIZATION_CHANGED")
