...
function RRT_NS:SetProperties(F, info, skipsound, s)
    ...
    F:SetScript("OnUpdate", function(_, elapsed)
        ...
        if F._updateAcc < 0.025 then return end  -- cap at 40 fps
        F._updateAcc = 0
...
function RRT_NS:AddTickToBar(F, percent)
...
function RRT_NS:MoveFrameInit(F, s, ReminderColor)
    if F then
        ...
        F.Label = F.Border:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
        F.Label:SetPoint("TOPLEFT", F.Border, "TOPLEFT", 4, -4)
        F.Label:SetFont(RRT_NS.LSM:Fetch("font", RRT.Settings.GlobalFont), 40, "OUTLINE")
