...
local function BuildEncounterAlertsUI(parent)
    local Opt = RRT_NS.UI.Options.EncounterAlerts
    local OptAssign = RRT_NS.UI.Options.Assignments
    ...
    local SECTIONS = {
        ...
        { name = L["tab_assignments"] or "Assignments", key = "assignments" },
        { name = "Custom Alert", key = "custom" },
    }

    local panels = {}
    for _, section in ipairs(SECTIONS) do
        local panel = CreateFrame("Frame", "RRTEncounterSection_" .. section.key, contentArea)
        panel:SetAllPoints(contentArea)
        panel:Hide()
        panel._rrtBuilt = false
        panels[section.key] = panel
    end

    local function BuildSection(key)
        local panel = panels[key]
        if not panel or panel._rrtBuilt then
            return
        end

        if key == "general" then
            DF:BuildMenu(panel, Opt.BuildGeneralOptions(), 10, -10, menuH, false,
        ...
        elseif key == "midnight" then
            DF:BuildMenu(panel, Opt.BuildMidnightOptions(), 10, -10, menuH, false,
                options_text_template, options_dropdown_template, options_switch_template,
                true, options_slider_template, options_button_template, Opt.BuildCallback())
        elseif key == "assignments" then
            DF:BuildMenu(panel, OptAssign.BuildMidnightOptions(), 10, -10, menuH, false,
                options_text_template, options_dropdown_template, options_switch_template,
                true, options_slider_template, options_button_template, OptAssign.BuildCallback())
        elseif key == "custom" then
            if RRT_NS.BuildCustomEncounterAlertsPanel then
                RRT_NS.BuildCustomEncounterAlertsPanel(panel)
            else
                DF:BuildMenu(panel, {
                    {
                        type = "label",
                        get = function() return "Custom Alert" end,
                        text_template = DF:GetTemplate("font", "ORANGE_FONT_TEMPLATE"),
                    },
                    {
                        type = "label",
                        get = function() return "No custom alert panel is available in this build." end,
                        spacement = true,
                    },
                }, 10, -10, menuH, false,
                    options_text_template, options_dropdown_template, options_switch_template,
    ...
    for i, section in ipairs(SECTIONS) do
        ...
        btn:SetScript("OnClick", function(self)
            SelectSection(key, self, sectionName)
        end)
        ...
        if i == 1 then
            SelectSection(key, btn, sectionName)
        end
    end
