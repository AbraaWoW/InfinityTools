...
local encID = 3179
-- /run RRTAPI:DebugEncounter(3179)
RRT_NS.EncounterAlertStart[encID] = function(self, id) -- on ENCOUNTER_START
    ...
    if RRT.EncounterAlerts[encID].enabled then -- text, Type, spellID, dur, phase, encID
        ...
        timers = {
            ...
            [16] = {17.1, 62.1, 140, 185.7, 261.5, 306.6},
        }
        ...
        timers = {
            ...
            [16] = {26.6, 72, 149.8, 195.9, 271.4, 316.5},
        }
    ...
    if RRT.EncounterAlerts[encID].CCAddsDisplay and id == 16 then
                ...
                for i=1, #self.platetexts+1 do
                    if self.platetexts[i] and not self.platetexts[i]:IsShown() then
                        ...
                        if issecretvalue(interruptible) then
                            if self.platetexts[i].SetAlphaFromBoolean then
                                self.platetexts[i]:SetAlphaFromBoolean(interruptible, 0, 1)
                            else
                                self.platetexts[i]:SetAlpha(interruptible and 0 or 1)
                            end
                            if self.platetexts[i].bgFrame.SetAlphaFromBoolean then
                                self.platetexts[i].bgFrame:SetAlphaFromBoolean(interruptible, 0, 1)
                            else
                                self.platetexts[i].bgFrame:SetAlpha(interruptible and 0 or 1)
                            end
                        end
                        return
                    elseif not self.platetexts[i] then
                        ...
                        self.platetexts[i]:SetFont(self.LSM:Fetch("font", RRT.Settings.GlobalFont), 18, "OUTLINE")
                        self.platetexts[i]:SetPoint("BOTTOM", plate, "TOP", 0, 0)
        ...
        local function UpdateNameplateTexts(e, u)
            ...
            elseif e == "NAME_PLATE_UNIT_ADDED" or e == "UNIT_AURA" or e == "UNIT_SPELLCAST_START" then
                ...
                if not found then return end -- only allow nameplate units for nameplate events
...
RRT_NS.EncounterAlertStop[encID] = function(self) -- on ENCOUNTER_END
    if RRT.EncounterAlerts[encID].CCAddsDisplay then
