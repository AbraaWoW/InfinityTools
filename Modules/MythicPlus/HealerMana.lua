...
local function GetHealers()
    ...
    if IsInRaid() then
        ...
        -- Toujours vérifier player + party1-4, IsInGroup() peut être unreliable en Midnight
...
local function RefreshDisplay()
    ...
    for i, unit in ipairs(healers) do
        ...
        local ok, val = pcall(function()
            -- raw 0 = Mana (évite Enum.PowerType.Mana qui peut être nil en Midnight)
            local maxM = UnitPowerMax(unit, 0)
            local curM = UnitPower(unit, 0)
            if maxM and maxM > 0 then
                return math.floor(curM / maxM * 100)
            end
            return nil
        end)
        if ok and val ~= nil then pct = val end
...
_ev:RegisterEvent("UNIT_POWER_UPDATE")
_ev:RegisterEvent("UNIT_MAXPOWER")
