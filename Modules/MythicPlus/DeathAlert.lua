...
local function OnEvent(self, event, ...)
    ...
    if event == "UNIT_DIED" then
        ...
        -- Use GetPlayerInfoByGUID to avoid secret value restriction on tainted unit tokens
        local localizedClass, _, _, _, _, name = GetPlayerInfoByGUID(destGUID)
        if not localizedClass then return end -- not a player
        if not name then name = "Unknown" end
        -- Check group membership via unit token (pcall in case still tainted)
        local unit = UnitTokenFromGUID(destGUID)
        local inGroup = false
        if unit then
            local ok, result = pcall(function()
                return UnitInRaid(unit) or UnitInParty(unit) or UnitIsUnit(unit, "player")
            end)
            inGroup = ok and result
        end
        -- Fallback: if unit token unusable, accept anyone GetPlayerInfoByGUID returned (they're in our group data)
        if not inGroup and not unit then return end
        local role = "damager"
        if unit then
            local ok, r = pcall(GetUnitRole, unit)
            if ok and r then role = r end
        end
        ShowAlert(name, role)
    end
end
