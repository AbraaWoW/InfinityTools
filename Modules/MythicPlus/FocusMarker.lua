...
-- ─────────────────────────────────────────────────────────────────────────────
local _btn = CreateFrame("Button", "RRTFocusMarkerBtn", _frame, "BackdropTemplate")
_btn:SetSize(110, 24)
_btn:SetPoint("BOTTOM", _frame, "BOTTOM", 0, 2)
_btn:EnableMouse(true)
_btn:RegisterForClicks("LeftButtonUp")
_btn:SetScript("OnClick", function()
    if InCombatLockdown() then return end
    if UnitExists("focus") then
        SetRaidTarget("focus", db().markerIndex or 8)
    end
end)
