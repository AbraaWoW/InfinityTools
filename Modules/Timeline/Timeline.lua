...
function RRT_NS:SetupTimelineHooks(timeline)
...
function RRT_NS:GetBossAbilityLines(encounterID, displayMode, requestedDifficulty)
...
function RRT_NS:GetMyTimelineData(includeBossAbilities, bossDisplayMode)
    ...
    local activeReminder = RRT.ActiveReminder
    local reminderSource = RRT.Reminders
    if not activeReminder or activeReminder == "" then
        activeReminder = RRT.ActivePersonalReminder
        reminderSource = RRT.PersonalReminders
    end
    ...
    for phase, reminders in pairs(self.ProcessedReminder[encID]) do
        ...
        for _, reminder in ipairs(reminders) do
            ...
            local dur = reminder.dur or (spellID and RRT.ReminderSettings.SpellDuration or RRT.ReminderSettings.TextDuration)
            ...
            if glowUnit and #glowUnit > 0 then
                for i, name in ipairs(glowUnit) do
                    glowUnitNames = glowUnitNames ..
                        RRTAPI:Shorten(RRTAPI:GetChar(name), 12, false, "GlobalNickNames") .. " "
                end
            else
                glowUnitNames = nil
            end
...
-- Returns data in DetailsFramework timeline format
-- bossDisplayMode: "all", "important", or "combined" (see BossDisplayModes)
function RRT_NS:GetAllTimelineData(reminderName, personal, includeBossAbilities, bossDisplayMode)
    local source = personal and RRT.PersonalReminders or RRT.Reminders
    ...
    for line in reminderStr:gmatch('[^\r\n]+') do
        ...
        if glowUnit then
            for name in glowUnit:gmatch("([^%s:]+)") do
                if name ~= "glowunit" then
                    glowUnitNames = glowUnitNames .. RRTAPI:Shorten(RRTAPI:GetChar(name), 12, false, "GlobalNickNames") .. " "
                end
            end
        else
            glowUnitNames = nil
        ...
        if tag and time then
            ...
            if dur then
                ...
                dur = spellID and RRT.ReminderSettings.SpellDuration or RRT.ReminderSettings.TextDuration
            end
    ...
    for _, player in ipairs(sortedPlayers) do
        ...
        for _, ability in ipairs(sortedAbilities) do
            ...
            local shortPlayer = RRTAPI:Shorten(player, 12, false, "GlobalNickNames") or player

            -- Get class color for the player
            local classColorHex = nil
            local unitName = RRTAPI:GetChar(player, true, "InfinityRaidTools")
...
function RRT_NS:CreateTimelineWindow()
    ...
    local timelineWindow = DF:CreateSimplePanel(UIParent, window_width, window_height,
        "|cFFBB66FFInfinity|r Timeline", "RRTTimelineWindow", {
        DontRightClickClose = true,
        UseStatusBar = false,
            UseScaleBar = true,
        },
        RRT.RRTUI.timeline_window)
    ...
    timelineWindow.bossDisplayMode = RRT_NS.BossDisplayModes.IMPORTANT_HEALER -- Default display mode
        ...
        return {
            {
                ...
                value = RRT_NS.BossDisplayModes.IMPORTANT_HEALER,
                onclick = function(_, _, value)
            ...
            },
            {
                ...
                value = RRT_NS.BossDisplayModes.IMPORTANT_TANK,
                onclick = function(_, _, value)
                    timelineWindow.bossDisplayMode = value
                    RRT_NS:RefreshTimelineForMode()
                end
            ...
            {
                label = "Show All",
                value = RRT_NS.BossDisplayModes.SHOW_ALL,
                onclick = function(_, _, value)
                    timelineWindow.bossDisplayMode = value
                    RRT_NS:RefreshTimelineForMode()
                end
            ...
            {
                label = "Combined",
                value = RRT_NS.BossDisplayModes.COMBINED,
                onclick = function(_, _, value)
                    timelineWindow.bossDisplayMode = value
                    RRT_NS:RefreshTimelineForMode()
                end
            },
            {
                ...
                value = RRT_NS.BossDisplayModes.COMBINED_IMPORTANT,
                onclick = function(_, _, value)
                    timelineWindow.bossDisplayMode = value
                    RRT_NS:RefreshTimelineForMode()
                end
    ...
    local bossDisplayDropdown = DF:CreateDropDown(timelineWindow, BuildBossDisplayModeOptions, RRT_NS.BossDisplayModes.IMPORTANT_HEALER, 150)
    ...
    local noDataLabel = DF:CreateLabel(timelineWindow, "No reminders to display. Load a reminder set first with /rvr", 14, "gray")
...
function RRT_NS:ToggleTimelineWindow()
...
function RRT_NS:RefreshTimelineForMode()
...
function RRT_NS:AutoFitTimelineScale(timeline, dataLength)
    if not timeline then return end
...
function RRT_NS:RefreshMyRemindersTimeline()
    ...
    if data and data.lines and #data.lines > 0 then
        ...
        self.TimelineWindow.noDataLabel:SetText("No reminders loaded for you.\nLoad a reminder set with /rvr and ensure it contains assignments for you.")
        self.TimelineWindow.noDataLabel:Show()
...
function RRT_NS:RefreshAllRemindersTimeline(reminderName, personal)
    if not self.TimelineWindow or not self.TimelineWindow.timeline then return end
...
function RRT_NS:UpdateTimelineTitle()
    local window = self.TimelineWindow
    if not window then return end

    local title = "|cFFBB66FFInfinity|r Timeline"
    ...
    if encID then
        ...
        if difficulty then
            title = string.format("|cFFBB66FFInfinity|r Timeline - %s (%s)", bossName, difficulty)
        else
            title = string.format("|cFFBB66FFInfinity|r Timeline - %s", bossName)
        end
    end
...
function RRT_NS:UpdatePhaseMarkers()
    local window = self.TimelineWindow
    if not window then return end
        ...
        if phaseNum > 1 then
            ...
            if not marker then
                ...
                marker:SetScript("OnDragStop", function(self)
                    ...
                    RRT_NS:SetPhaseStart(self.encID, self.phaseNum, newTime)
                ...
                marker:SetScript("OnEnter", function(self)
                    ...
                    local time = RRT_NS:GetPhaseStart(self.encID, self.phaseNum)
                ...
                marker:SetScript("OnMouseDown", function(self, button)
                    if button == "RightButton" then
                        RRT_NS:ResetPhaseStart(self.encID, self.phaseNum)
                        RRT_NS:RefreshTimelineForMode()
                    end
                end)
...
--------------------------------------------------------------------------------
-- EMBEDDED TIMELINE FUNCTIONS (for RRTUI tab)
--------------------------------------------------------------------------------

-- Refresh the embedded timeline based on current mode
function RRT_NS:RefreshEmbeddedTimeline(tab)
...
function RRT_NS:RefreshEmbeddedMyReminders(tab)
    if not tab or not tab.timeline then return end
...
function RRT_NS:RefreshEmbeddedAllReminders(tab, reminderName, personal)
    if not tab or not tab.timeline then return end
...
function RRT_NS:UpdateEmbeddedTimelineTitle(tab)
...
function RRT_NS:UpdateEmbeddedPhaseMarkers(tab)
        ...
        if phaseNum > 1 then
            ...
            if not marker then
                ...
                marker:SetScript("OnDragStop", function(self)
                    ...
                    RRT_NS:SetPhaseStart(self.encID, self.phaseNum, newTime)
                    RRT_NS:RefreshEmbeddedTimeline(self.parentTab)
                end)
